...
async function runWithRetry(courseId) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    console.log(`[Academy Studio] Zero-cost local research starting ${courseId}, attempt ${attempt}/${maxAttempts}.`);
...
for (let index = 0; index < Math.min(canaryCount, queue.length); index += 1) {
  ...
  console.log(`[Academy Studio] Local research canary ${index + 1}/${canaryCount} assigned ${courseId}.`);
  const result = await runWithRetry(courseId);
  results.push({ ...result, reused: false, canary: true });
  if (!result.ok) {
    canaryFailed = true;
    circuitOpen = true;
    console.error(`[Academy Studio] Local research canary failed for ${courseId}; batch research is halted without external paid fallback.`);
    break;
  }
}
...
async function worker(workerId) {
  while (queue.length > 0 && !circuitOpen) {
    const courseId = queue.shift();
    if (!courseId) return;
    console.log(`[Academy Studio] Zero-cost local research worker ${workerId}/${concurrency} assigned ${courseId}.`);
    const result = await runWithRetry(courseId);
...
for (const courseId of deferred) {
  results.push({ courseId, attempt: 0, ok: false, reused: false, canary: false, deferred: true, code: null, error: canaryFailed ? "not-started-local-canary-failed" : "not-started-local-model-circuit-open" });
}
...
const summary = {
  ...
  localResearchConcurrency: concurrency,
  localCanaryCount: canaryCount,
...
console.log(`[Academy Studio] Zero-cost authoritative research ready for ${summary.completed}/61 courses: ${summary.reused} reused, ${summary.newlyCompleted} newly researched, ${summary.deferred} deferred.`);
