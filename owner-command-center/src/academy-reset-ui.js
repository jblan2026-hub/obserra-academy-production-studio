...
(() => {
  ...
  function cls(value) {
    ...
    if (["blocked", "failed", "rejected", "retired", "verification-failed"].includes(v)) return "bad";
    return "neutral";
  }
  ...
  function renderDetail() {
    ...
    el.detail.innerHTML = `
      ...
      <section><div class="sectionTitle"><div><p class="eyebrow">SECURE PURCHASE VERIFICATION</p><h3>Validate a real paid enrollment</h3></div></div><p class="boundary">Enter a Stripe PaymentIntent (pi_...) or Checkout Session (cs_...). The reference is used at runtime only. The console does not retain customer email, card data, payment-method details, raw Stripe/Clerk customer records, or the full transaction reference in owner-facing evidence.</p><div class="paymentRow"><input id="paymentReference" type="password" autocomplete="off" spellcheck="false" placeholder="pi_... or cs_..." /><button data-purchase-action="verify">Verify paid access</button></div></section>
    `;
  }

  function renderEvidence() {
    const value = state.lastError ? { state: "error", message: state.lastError } : state.lastResult || { state: "ready", message: "Select a course and action.", privacy: "Sensitive customer and payment data is excluded from owner-facing evidence." };
  ...
  async function refresh() {
    try {
      const [endpoint, snapshot] = await Promise.all([window.obserraOwner.getEndpointSnapshot(), window.obserraOwner.getAcademyControlSnapshot()]);
      state.endpoint = endpoint;
      state.snapshot = snapshot;
      if (!state.selectedCourseId || !snapshot.courses?.some((course) => course.id === state.selectedCourseId)) state.selectedCourseId = snapshot.courses?.[0]?.id || null;
  ...
  async function openPreview(kind) {
    const course = selected();
    if (!course) return;
    const operation = kind === "course" ? window.obserraOwner.previewAcademyCourse : kind === "materials" ? window.obserraOwner.previewAcademyMaterials : window.obserraOwner.previewAcademyCertificate;
    try {
  ...
  el.detail.addEventListener("click", async (event) => {
    ...
    if (review) {
      ...
      if (!note) return;
      return execute("review-decision", () => window.obserraOwner.updateAcademyReview({ courseId: course.id, reviewName: review.dataset.reviewName, decision: review.dataset.reviewDecision, note }));
    }
    const courseAction = event.target.closest("[data-course-action]");
    if (courseAction) return execute(`course-${courseAction.dataset.courseAction}`, () => window.obserraOwner.runAcademyControlledAction({ action: courseAction.dataset.courseAction, courseId: course.id }));
    ...
    if (releaseAction) {
      ...
      return execute(`release-${action}`, () => window.obserraOwner.transitionAcademyCourse({ courseId: course.id, action, note, confirmation }));
    }
    const verify = event.target.closest("[data-purchase-action='verify']");
    if (verify) {
      const input = document.getElementById("paymentReference");
      const reference = input?.value?.trim();
      if (!reference) { state.lastError = "Enter a Stripe PaymentIntent or Checkout Session reference."; return renderEvidence(); }
      input.value = "";
      return execute("verify-paid-access", () => window.obserraOwner.verifyAcademyPurchase({ courseId: course.id, reference }));
    }
  });

  window.setInterval(() => { if (document.visibilityState === "visible" && !state.busy) refresh(); }, 30000);
  document.addEventListener("visibilitychange", () => { if (document.visibilityState === "visible" && !state.busy) refresh(); });
  refresh();
})();
