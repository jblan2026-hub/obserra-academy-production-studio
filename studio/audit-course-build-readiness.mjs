...
function parseDurationMinutes(value) {
  ...
  const hours = normalized.match(
    /([0-9]+(?:\.[0-9]+)?)\s*(?:hour|hours|hr|hrs)/,
  );
  const minutes = normalized.match(
    /([0-9]+(?:\.[0-9]+)?)\s*(?:minute|minutes|min|mins)/,
  );
  if (hours || minutes) {
    return Math.round(
      Number(hours?.[1] ?? 0) * 60 + Number(minutes?.[1] ?? 0),
    );
  }
...
if (!fs.existsSync(coursesRoot)) {
  throw new Error(`Courses directory not found: ${coursesRoot}`);
}
...
for (const entry of fs
  ...
  const publicationApproved =
    ...
    ["approved", "published"].includes(releaseStatus);
  const ownerReviewEligible = !["retired", "archived"].includes(releaseStatus);
  const courseFindings = [];

  if (courseId !== entry.name) {
    courseFindings.push("directory-course-id-mismatch");
  }
  for (const field of [
    "id",
    "title",
    ...
    if (!String(course[field] ?? "").trim()) {
      courseFindings.push(`missing-course-${field}`);
    }
  }
  if (!Array.isArray(course.outcomes) || course.outcomes.length === 0) {
    courseFindings.push("missing-course-outcomes");
  }
  ...
  for (const [index, module] of modules.entries()) {
    for (const field of ["id", "title", "description", "duration", "format"]) {
      if (!String(module?.[field] ?? "").trim()) {
        courseFindings.push(`module-${index + 1}-missing-${field}`);
      }
    }
    if (moduleIds.has(module.id)) {
      courseFindings.push(`module-${index + 1}-duplicate-id`);
    }
    moduleIds.add(module.id);
    const minutes = parseDurationMinutes(module.duration);
    if (!Number.isFinite(minutes) || minutes <= 0) {
      courseFindings.push(`module-${index + 1}-invalid-duration`);
    } else {
      moduleMinutes += minutes;
    }
  }

  const assessmentMinutes = parseDurationMinutes(
    manifest.completion?.assessmentDuration,
  );
  const accountedMinutes =
    moduleMinutes +
    (Number.isFinite(assessmentMinutes) && assessmentMinutes > 0
      ? assessmentMinutes
      : 0);
  const advertisedMinutes = parseDurationMinutes(course.duration);
  if (!Number.isFinite(advertisedMinutes) || advertisedMinutes <= 0) {
    courseFindings.push("invalid-course-duration");
  } else if (accountedMinutes !== advertisedMinutes) {
    courseFindings.push(
      `duration-mismatch-${accountedMinutes}-vs-${advertisedMinutes}`,
    );
  }
  ...
  if (!manifest.completion?.allLessonsRequired) {
    courseFindings.push("all-lessons-not-required");
  }
  if (!manifest.completion?.assessmentRequired) {
    courseFindings.push("assessment-not-required");
  }
  if (
    !Number.isFinite(Number(manifest.completion?.passingScore)) ||
    Number(manifest.completion.passingScore) < 1
  ) {
    courseFindings.push("invalid-passing-score");
  }
  if (manifest.completion?.certificateIssued !== true) {
    courseFindings.push("certificate-not-enabled");
  }

  if (manifest.branding?.logoAsset !== officialBrand.officialLogo.assetPath) {
    courseFindings.push(
      publicationApproved
        ? "official-logo-mismatch"
        : "draft-branding-not-applied",
    );
  }

  const missingGenerated = requiredGeneratedFiles.filter(
    (name) => !fs.existsSync(path.join(courseDir, name)),
  );
  if (missingGenerated.length) {
    courseFindings.push(
      ...missingGenerated.map((name) => `missing-generated-${name}`),
    );
  }
  ...
  if (!authoringMissing) {
    ...
    if (!packageManifestHash) {
      courseFindings.push("untraceable-ai-course-package");
    } else if (packageManifestHash !== manifestHash) {
      courseFindings.push("stale-ai-course-package");
    }
    if (packageAuthoringPolicyVersion !== ACADEMY_AUTHORING_POLICY_VERSION) {
      courseFindings.push("outdated-ai-authoring-policy");
    }
    packageQualityFindings = authoredPackageFindings({
      manifest,
      authored: authored.content,
    });
    if (packageQualityFindings.length > 0) {
      courseFindings.push("invalid-ai-course-package");
    }
  }
  ...
  const courseBlockingFindings = courseFindings.filter(
    (finding) => !nonBlockingFindings.has(finding),
  );
  ...
  courses.push({
    ...
    moduleMinutes,
    assessmentMinutes: Number.isFinite(assessmentMinutes)
      ? assessmentMinutes
      : 0,
    accountedMinutes,
    advertisedMinutes,
    ...
    packageQualityFindingCount: packageQualityFindings.length,
    packageQualityFindings,
    authoringMissing,
...
const publicationApprovedCourses = courses.filter(
  (course) => course.publicationApproved,
);
const ownerReviewCourses = courses.filter((course) => course.ownerReviewEligible);
const expectedOwnerReviewCourses = Number(
  process.env.ACADEMY_EXPECTED_REVIEW_COURSES ?? 0,
);
if (
  Number.isInteger(expectedOwnerReviewCourses) &&
  expectedOwnerReviewCourses > 0 &&
  ownerReviewCourses.length !== expectedOwnerReviewCourses
) {
  findings.push({
...
const buildRequired = ownerReviewCourses.some(
  (course) =>
    course.blockingFindings.length > 0 ||
    course.findings.some((finding) =>
      finding.startsWith("missing-generated-"),
    ),
);
...
const report = {
  ...
  policy: {
    ...
    duration:
      "module-durations-plus-explicit-final-assessment-duration-must-equal-advertised-course-duration",
    draftCommerce:
      "draft courses may retain a zero placeholder until owner and commerce approval",
    releaseBranding:
      "approved and published courses must use the owner-approved official logo",
    authoring:
      "all owner-review-eligible missing, stale, untraceable, older-policy, or production-depth-invalid packages trigger AI authoring",
    build:
      "all owner-review-eligible missing assets or blocking findings trigger governed build",
    publication:
      "only explicitly approved or published courses can enter the public catalog",
    directProductionPublish: false,
  },
  totals: {
    discovered: courses.length,
    ownerReviewEligible: ownerReviewCourses.length,
    expectedOwnerReviewEligible:
      Number.isInteger(expectedOwnerReviewCourses) &&
      expectedOwnerReviewCourses > 0
        ? expectedOwnerReviewCourses
        : null,
...
fs.writeFileSync(
  path.join(catalogRoot, "continuous-course-audit.json"),
...
writeOutput(
  "publication_approved_courses",
  String(publicationApprovedCourses.length),
);
writeOutput("approved_courses", String(publicationApprovedCourses.length));

console.log(
  `[Academy Studio] Audited ${courses.length} course manifests under authoring policy ${ACADEMY_AUTHORING_POLICY_VERSION}, including ${ownerReviewCourses.length} owner-review-eligible and ${publicationApprovedCourses.length} publication-approved course(s).`,
);
console.log(
  `[Academy Studio] AI authoring required: ${authoringRequired}. Governed build required: ${buildRequired}. Blocking findings: ${blockingFindings.length}.`,
);
if (blockingFindings.length > 0) {
  for (const item of blockingFindings.slice(0, 100)) {
    console.error(`[Academy Studio] ${item.courseId}: ${item.finding}`);
  }
  process.exitCode = 2;
}
