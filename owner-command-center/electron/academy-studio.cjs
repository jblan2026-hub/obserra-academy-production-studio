...
function runStudioAction(action, courseId) {
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      finish({ ok: false, exitCode: null, signal: null, stderr: error instanceof Error ? error.message : String(error) });
      return;
    }
    ...
    timeoutTimer = setTimeout(() => {
      ...
      settleTimer = setTimeout(() => {
        finish({ ok: false, exitCode: child.exitCode, signal: child.signalCode || null });
      }, ACTION_TERMINATION_GRACE_MS + ACTION_SETTLE_GRACE_MS);
    }, timeoutMs);

    child.on("error", (error) => {
      stderr = appendBoundedOutput(stderr, error instanceof Error ? error.message : String(error));
