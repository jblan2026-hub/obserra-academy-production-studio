...
function workspaceCandidates() {
  ...
  return [process.env.OBSERRA_ACADEMY_STUDIO_ROOT, path.resolve(__dirname, "../.."), path.join(home, "source", "repos", "obserra-academy-production-studio"), path.join(home, "Documents", "GitHub", "obserra-academy-production-studio"), path.join(home, "GitHub", "obserra-academy-production-studio")].filter(Boolean);
}
function isStudioRoot(candidate) { return Boolean(candidate && fs.existsSync(path.join(candidate, "package.json")) && fs.existsSync(path.join(candidate, "courses")) && fs.existsSync(path.join(candidate, "studio"))); }
function resolveStudioRoot() { const configured = workspaceCandidates().find(isStudioRoot); return configured ? fs.realpathSync(configured) : null; }
function assertCourseId(value) { if (typeof value !== "string" || !/^[a-z0-9][a-z0-9-]{1,120}$/.test(value)) throw new Error("Invalid course identifier"); return value; }
function readJson(filePath) { return JSON.parse(fs.readFileSync(filePath, "utf8")); }
function atomicWriteJson(filePath, value) { const tempPath = `${filePath}.${process.pid}.${Date.now()}.tmp`; fs.writeFileSync(tempPath, `${JSON.stringify(value, null, 2)}\n`, { encoding: "utf8", mode: 0o600 }); fs.renameSync(tempPath, filePath); }
...
function summarizeCourse(root, courseId) {
  ...
  return { id: manifest.course.id, title: manifest.course.title, department: manifest.course.department, level: manifest.course.level, track: manifest.course.track, description: manifest.course.description, duration: manifest.course.duration, price: manifest.commerce?.price ?? null, currency: manifest.commerce?.currency ?? "USD", moduleCount: Array.isArray(manifest.course.modules) ? manifest.course.modules.length : 0, releaseStatus: manifest.release?.status || "draft", publishToAcademy: manifest.release?.publishToAcademy === true, version: manifest.release?.version || "0.0.0", generation: fs.existsSync(generatedPackage) ? "generated" : "not-generated", finalRelease: fs.existsSync(finalRelease), queueStatus: queue?.status || "not-queued", artifacts: artifactStatus, missingArtifacts, reviews: reviewEntries, reviewCompletion: requiredReviews.length ? Math.round((completedReviews.length / requiredReviews.length) * 100) : 100, recommendations };
}
...
function getStudioSnapshot() {
  ...
  return { available: true, mode: "local-workspace", root, checkedAt: new Date().toISOString(), courses, summary: { total: courses.length, generated: courses.filter((course) => course.generation === "generated").length, published: courses.filter((course) => course.publishToAcademy && ["approved", "published"].includes(course.releaseStatus)).length, reviewReady: courses.filter((course) => course.generation === "generated" && course.missingArtifacts.length === 0).length, gaps: courses.reduce((count, course) => count + course.recommendations.length, 0) }, gaps };
}
...
function updateCourseMetadata(payload) {
  ...
  if (typeof updates.releaseStatus === "string") { if (!ALLOWED_RELEASE_STATUSES.has(updates.releaseStatus)) throw new Error("Unsupported release status"); manifest.release.status = updates.releaseStatus; }
  if (typeof updates.publishToAcademy === "boolean") { if (updates.publishToAcademy && !["approved", "published"].includes(manifest.release.status)) throw new Error("Only approved or published courses may be enabled for Academy publication"); manifest.release.publishToAcademy = updates.publishToAcademy; }
...
function runStudioAction(action, courseId) {
  ...
  const commandMap = {
    ...
    revise: ["run", "author:course", "--", "--course", assertCourseId(courseId), "--force"],
