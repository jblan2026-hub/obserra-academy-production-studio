...
const summaryPath = path.join(catalogRoot, "academy-61-media-submission.json");
...
const expectedCourses = Number(process.env.ACADEMY_EXPECTED_REVIEW_COURSES || process.env.ACADEMY_EXPECTED_SURGE_COURSES || 61);
...
if (!Number.isInteger(expectedCourses) || expectedCourses !== 61) throw new Error(`All-course media lane requires exactly 61 courses; configured ${expectedCourses}.`);
if (!templateApproved) {
...
if (eligibleCourses.length !== expectedCourses) {
  throw new Error(`Media submission requires exactly ${expectedCourses} compliance-staged courses; discovered ${eligibleCourses.length}.`);
}
...
const summary = {
  ...
  publicationAuthorized: false,
  results,
  claimBoundary: "An accepted provider job is not a mastered instructional video. Final media requires provider completion, asset retrieval, segment assembly, audible narration verification, source cards, captions, transcripts, audio description or approved alternatives, rights evidence, technical quality control, content review, and owner acceptance."
};
fs.mkdirSync(catalogRoot, { recursive: true });
writeJson(summaryPath, summary);
console.log(`[Academy Studio] All-course cinematic media queue reconciled ${jobs.length} job(s): ${submitted.length} new, ${preserved.length} preserved, ${configurationRequired.length} configuration required, ${failed.length} failed.`);
