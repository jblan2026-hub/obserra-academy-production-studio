...
archive="${HOME}/.cache/obserra-academy/ollama/ollama-linux-amd64-${OLLAMA_VERSION}.tar.zst"
test -s "$archive"
...
nohup env \
  ...
  ollama serve > /tmp/academy-ollama.log 2>&1 &
...
nohup node studio/local-ollama-keepalive-proxy.mjs > /tmp/academy-ollama-proxy.log 2>&1 &
for _attempt in $(seq 1 60); do
  if curl --fail --silent "http://127.0.0.1:${ACADEMY_OLLAMA_PROXY_PORT:-11436}/health" >/dev/null; then
    break
  fi
  sleep 1
done
curl --fail --silent "http://127.0.0.1:${ACADEMY_OLLAMA_PROXY_PORT:-11436}/health"
