...
function requirePattern(content, pattern, description) {
  if (!pattern.test(content)) throw new Error(`Local-only verification failed: ${description}`);
}
...
if (packageJson.private !== true) throw new Error("Local-only verification failed: package must remain private");
if (!packageJson.build || packageJson.build.publish) throw new Error("Local-only verification failed: automatic public publishing must not be configured");
const resources = connectorCatalog.resources;
if (!Array.isArray(resources) || resources.length === 0) throw new Error("Local-only verification failed: approved resource catalog is missing or empty");
if (!String(connectorCatalog.defaultControlMode ?? "").toLowerCase().includes("read")) throw new Error("Local-only verification failed: catalog default control mode must be read-only");
for (const resource of resources) {
  if (!resource.id || !resource.name || !resource.type) throw new Error("Local-only verification failed: every resource requires an id, name, and type");
  if (resource.writeCapabilitiesRequireOwnerApproval !== true) throw new Error(`Local-only verification failed: resource ${resource.id} must require owner approval for write capabilities`);
  if (!Array.isArray(resource.capabilities) || resource.capabilities.length === 0) throw new Error(`Local-only verification failed: resource ${resource.id} must declare capabilities`);
}
