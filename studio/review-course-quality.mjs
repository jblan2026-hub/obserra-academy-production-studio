...
if (!apiKey) throw new Error("OPENAI_API_KEY is required for independent course quality review.");
...
if (!response.ok) throw new Error(`Independent course review failed with ${response.status}: ${raw.slice(0, 3000)}`);
const payload = JSON.parse(raw);
if (payload.status === "incomplete") throw new Error(`Independent course review response incomplete: ${JSON.stringify(payload.incomplete_details || {})}`);
const text = outputText(payload);
if (!text) throw new Error("Independent course review returned no output text.");
...
console.log(`[Academy Studio] Independent quality review ${evidence.passed ? "PASSED" : "FAILED"} for ${courseId}.`);
