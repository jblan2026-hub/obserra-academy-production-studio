...
const cases = Array.from({ length: 2000 }, (_, index) => {
  ...
  return { findingId, target: targets[index % targets.length], flow: flows[index % flows.length], mapping: mappings[index % mappings.length], severity: index % 11 === 0 ? "critical" : "high", knownBad: true, stages: [...stages], ownerApproval: { required: true, decision: "approved", noteRequired: true }, branch: `ai-remediation/${findingId}`, draftPullRequestOnly: true, sourceHashRequired: true, validationRequired: true, rollback: { required: true, beforeSha256: digest({ findingId, state: "before" }), afterSha256: digest({ findingId, state: "after" }) }, directDefaultBranchWriteAllowed: false, forcePushAllowed: false, automaticMergeAllowed: false, automaticProductionDeploymentAllowed: false };
});
...
for (const relative of ["owner-command-center/electron/main-with-remediation.cjs", "owner-command-center/electron/remediation-queue.cjs", "owner-command-center/electron/ai-remediation.cjs", "owner-command-center/electron/vulnerability-scan.cjs", "owner-command-center/scripts/verify-remediation-runtime.mjs", "studio/verify-1000x-remediation-workload.mjs"]) check(`required resilience surface ${relative}`, fs.existsSync(path.join(root, relative)));
const workloadDigest = digest(cases);
check("deterministic workload digest", workloadDigest.length === 64);
console.log(JSON.stringify({ gate: "studio-command-center-cross-project-resilience-2000x", cases: cases.length, targets, flows, mappings, stages, digest: workloadDigest, failures }, null, 2));
